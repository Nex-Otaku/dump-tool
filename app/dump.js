...
const applyDump = async () => {
    ...
    const passwordOption = local.password.length === 0 ? '' : ' --password="{password}"';
...
const dumpData = async () => {
    ...
    const dumpCommand = 'mysqldump --complete-insert --no-create-info -u {user} -p"{password}" -h {host} --port {port} --single-transaction --skip-lock-tables --default-character-set=utf8mb4 --hex-blob --max-allowed-packet=512000000 {db} {table} > {dumpFilePath}';
    const dumpCommandParametrized = lib.parametrize(dumpCommand, {
        host: remote.host,
        port: remote.port,
